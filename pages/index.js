...
export default function Home() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black"
          style={{
            'background-image':
              'linear-gradient(rgba(0 0 0 / 60%), rgba(0 0 0 / 60%)), url(/hero-bg.jpeg)'}}>
      <Head>
        <title>Netflix Next.js clone</title>
      ...
      <header className="flex justify-between w-full px-12 py-8">
        ...
        <div>
            {!session ? 
              <button 
                onClick={() => signIn()} 
                className="flex items-center bg-red-700 text-white text-small px-4 py-2 rounded">
                Sign in
              </button> 
                : 
              <button 
      ...
      <main className="flex flex-col items-center justify-center w-full flex-1 px-20 text-center">
        ...
        <div className="flex mt-4">
          <input placeholder="Email address" className="bg-white p-4 min-w-400"/>
          <button onClick={() => signIn()} className="flex items-center bg-red-700 text-white text-xl px-8">Get Started
